                       ...
                       uint64_t *posArray, uint64_t *dsink)
{
  ...
  if (tid < THREADS_NUM)
  {
    ...
    asm volatile("{\t\n"
                 ...
                 "}"
                 : "=l"(ptr1) // 第一个（输出）操作数，用%0表示->rd
                 : "l"(ptr)   // 第二个（输入）操作数，用%1表示->rs1
                 : "memory"); // **关键：内存破坏约束**
    // ld.global.ca.u64 rd, [rs1];
    // 效果：rd = *(u64*)rs1; // 从全局内存加载64位数据到寄存器rd，使用ca缓存修饰符
...
float l1_lat(int argc, char *argv[])
{
  ...
  printf("l1_lat<<<config.BLOCKS_NUM:%u, THREADS_PER_BLOCK:%u>>>\n", config.BLOCKS_NUM, config.THREADS_PER_BLOCK);
  l1_lat<<<config.BLOCKS_NUM, config.THREADS_PER_BLOCK>>>(startClk_g, stopClk_g, posArray_g, dsink_g);
